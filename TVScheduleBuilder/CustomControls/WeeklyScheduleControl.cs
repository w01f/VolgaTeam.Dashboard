                    ...
                    for (int k = 0; k < (totalSpotsCount == 0 ? 1 : totalSpotsCount); k += 13)
                    {
                        ...
                        for (int j = 0; j < programsPerSlide; j++)
                        {
                            if ((i + j) < _localSchedule.WeeklySchedule.Programs.Count)
                            {
                                ...
                                if (FormMain.Instance.buttonItemWeeklyScheduleRate.Checked)
                        ...
                        outputPage.TotalGRP = FormMain.Instance.buttonItemWeeklyScheduleCPP.Checked ? _localSchedule.WeeklySchedule.TotalGRP.ToString(_localSchedule.RatingAsCPP ? "#,###.0" : "#,##0") : string.Empty;
                        #endregion
                ...
                if (program != null)
                {
                    using (ToolForms.FormProgress form = new ToolForms.FormProgress())
                    {
                        ...
                        using (ToolForms.FormSelectOutput formSelect = new ToolForms.FormSelectOutput())
                        {
                            ...
                            formSelect.buttonXGrid.Enabled = _localSchedule.WeeklySchedule.Programs.Count <= 4 && outputPage.SpotsPerSlide <= 13 && _localSchedule.WeeklySchedule.Programs.Count <= 4 && File.Exists(Path.Combine(BusinessClasses.OutputManager.Instance.OneSheetTableBasedTemplatesFolderPath, string.Format(BusinessClasses.OutputManager.OneSheetTableBasedTemplateFileName, new object[] { outputPage.ProgramsPerSlide.ToString(), outputPage.SpotsPerSlide.ToString() })));
