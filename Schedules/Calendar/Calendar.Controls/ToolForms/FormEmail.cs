			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							CalendarPowerPointHelper.Instance.CreateLockedPresentation(PresentationFile.Replace(Path.GetExtension(PresentationFile), string.Empty), emailFile);
			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							CalendarPowerPointHelper.Instance.ConvertToPDF(PresentationFile, emailFile);
							if (File.Exists(emailFile))
							{
								if (OutlookHelper.Instance.Open())
								{
