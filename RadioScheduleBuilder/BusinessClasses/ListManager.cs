...
namespace RadioScheduleBuilder.BusinessClasses
{
    public class ListManager
    {
        ...
        private const string RadioStrategyFileName = @"Radio XML\Radio Strategy.xml";
        ...
        private void LoadRadioStrategy()
        {
            ...
            this.Dayparts.Clear();
            this.Times.Clear();
            string listPath = Path.Combine(this.ListsFolder, RadioStrategyFileName);
            if (File.Exists(listPath))
            {
                XmlDocument document = new XmlDocument();
                document.Load(listPath);

                XmlNode node = document.SelectSingleNode(@"/RadioStrategy");
                if (node != null)
                {
                    foreach (XmlNode childeNode in node.ChildNodes)
                    {
