...
namespace NewBizWiz.AdSchedule.Controls.InteropClasses
{
	...
	{
		public void AppendCalendar(MonthViewControl[] monthViews, Presentation destinationPresentation = null)
		{
			...
			{
				monthView.PrepareOutput();
				var presentationTemplatePath = Path.Combine(BusinessWrapper.Instance.OutputManager.CalendarTemlatesFolderPath, monthView.SlideName);
		...
		{
			PreparePresentation(fileName, presentation => AppendCalendar(outputControls, presentation));
		}
	}
}