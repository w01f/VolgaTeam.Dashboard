...
namespace CalendarBuilder.PresentationClasses.Views.MonthView
{
	...
	public partial class MonthViewControl : UserControl, IView
	{
		...
		public MonthViewControl(ICalendarControl calendar)
		{
			...
			CopyPasteManager.OnResetPaste += (sender, e) => { CalendarVisualizer.Instance.PasteButtonItem.Enabled = false; };
			CopyPasteManager.DayCopied += (sender, e) => { CalendarVisualizer.Instance.PasteButtonItem.Enabled = true; };

			CopyPasteManager.DayPasted += (sender, e) =>
											  {
												  Calendar.DayProperties.LoadData();
		...
		public void LoadData(bool reload = false)
		{
			if (!reload)
			{
				...
				{
					var month = new MonthControl(Calendar.CalendarData.Schedule.SundayBased);
		...
		{
			foreach (MonthControl month in Months.Values)
				month.RefreshData();
		}
																					  ...
																					  foreach (var weekDays in datesByWeeks)
																					  {
																						  ...
																						  foreach (DateTime weekDay in weekDays)
																						  {
																							  ...
																							  if (calendarDay != null)
																							  {
																								  ...
																								  dayControl.PropertiesRequested += (sender, e) => { Calendar.DayProperties.Show(); };
																								  ...
																								  dayControl.DataChanged += (sender, e) => { Calendar.SettingsNotSaved = true; };
