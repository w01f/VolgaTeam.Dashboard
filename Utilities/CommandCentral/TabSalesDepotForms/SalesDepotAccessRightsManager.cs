...
{
	internal class SalesDepotAccessRightsManager
	{
			...
			if (connection.State == ConnectionState.Open)
			{
				...
				try
				{
					...
					if (dataTable.Rows.Count > 0 && dataTable.Columns.Count > 2)
						...
						{
							SalesDepotUserLibraries approvedLibrariesForUser = approvedLibraries.Where(x => x.AppID.Equals(Guid.Empty) && x.UserName.Equals(DefaultUserName)).FirstOrDefault();
							if (approvedLibrariesForUser == null)
							{
								approvedLibrariesForUser = new SalesDepotUserLibraries();
					...
					if (dataTable.Rows.Count > 0 && dataTable.Columns.Count > 2)
						foreach (DataRow row in dataTable.Rows)
						{
							...
							if (Guid.TryParse(row[0].ToString().Trim().ToLower().Replace("appid-", string.Empty), out tempGuid))
							{
								...
								SalesDepotUserLibraries approvedLibrariesForUser = approvedLibraries.Where(x => x.AppID.Equals(tempGuid) && x.UserName.Equals(userName)).FirstOrDefault();
								if (approvedLibrariesForUser == null)
								{
									approvedLibrariesForUser = new SalesDepotUserLibraries();
				...
				foreach (SalesDepotUserLibraries approvedLibrariesForUser in approvedLibraries)
				{
					xml.Append(@"<User ");
					xml.Append("Name = \"" + approvedLibrariesForUser.UserName.Replace(@"&", "&#38;").Replace(@"<", "&#60;").Replace("\"", "&quot;") + "\" ");
					xml.Append("AppID = \"" + approvedLibrariesForUser.AppID + "\" ");
					xml.Append("UseRemoteLibraries = \"" + approvedLibrariesForUser.UseRemoteLibraries + "\" ");
					xml.AppendLine(@">");
