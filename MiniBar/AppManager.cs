...
namespace MiniBar
{
	internal class AppManager
	{
		...
		{
			foreach (var process in Process.GetProcesses().Where(x => x.ProcessName.ToUpper().Equals("POWERPNT")))
				process.Kill();
		}

		public void KillExcel()
		{
			foreach (var process in Process.GetProcesses().Where(x => x.ProcessName.ToUpper().Equals("EXCEL")))
				process.Kill();
		}

		public void KillFMAutoSync()
		{
			foreach (var process in Process.GetProcesses().Where(x => x.ProcessName.ToUpper().Equals("AUTOFMSYNC")))
				process.Kill();
		}
		#endregion
	}
