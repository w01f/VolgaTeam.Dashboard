...
namespace CalendarBuilder.InteropClasses
{
	public partial class PowerPointHelper
	{
		...
		{
			if (Directory.Exists(OutputManager.Instance.CalendarTemlatesFolderPath))
			{
				string presentationTemplatePath = Path.Combine(OutputManager.Instance.CalendarTemlatesFolderPath, monthOutputData.SlideName);
				if (File.Exists(presentationTemplatePath))
				{
					try
					{
						var thread = new Thread(delegate()
						{
							...
							var backgroundFilePath = Path.Combine(OutputManager.Instance.CalendarBackgroundFolderPath, String.Format(OutputManager.BackgroundFilePath, monthOutputData.SlideColor, monthOutputData.Parent.Date.ToString("yyyy")), monthOutputData.BackgroundFileName);
