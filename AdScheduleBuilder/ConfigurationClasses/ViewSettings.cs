...
namespace AdScheduleBuilder.ConfigurationClasses
{
	public class ScheduleBuilderViewSettings
	{
		public ScheduleBuilderViewSettings()
		{
			...
			ChronoGridViewSettings = new ChronoGridViewSettings();
		...
		public string Serialize()
		{
			...
			result.AppendLine(@"<ChronoGridViewSettings>" + ChronoGridViewSettings.Serialize() + @"</ChronoGridViewSettings>");
			...
			foreach (XmlNode childNode in node.ChildNodes)
			{
				switch (childNode.Name)
				{
					...
					case "ChronoGridViewSettings":
						ChronoGridViewSettings.Deserialize(childNode);
						break;
		...
		public void SaveDefaultViewSettings()
		{
			...
			using (var sw = new StreamWriter(SettingsManager.Instance.LocalSettingsPath, false))
			{
				sw.Write(xml);
				sw.Flush();
			}
	...
	public class ChronoGridViewSettings
	{
		public ChronoGridViewSettings()
		{
			...
			SlideBulletsState = new SlideBulletsState();
			SlideHeaderState = new SlideHeaderState();
			GridColumnsState.ShowPublication = true;
			GridColumnsState.IDPosition = -1;
			GridColumnsState.DatePosition = 0;
			GridColumnsState.PublicationPosition = 1;
		...
		{
			XmlNode node;
			if (File.Exists(SettingsManager.Instance.LocalSettingsPath))
			{
				var document = new XmlDocument();
				document.Load(SettingsManager.Instance.LocalSettingsPath);
		...
		public string Serialize()
		{
			...
			result.AppendLine(@"<SelectedOptionChapterIndex>" + SelectedOptionChapterIndex.ToString() + @"</SelectedOptionChapterIndex>");
			result.AppendLine(@"<SlideHeader>" + SlideHeader.Replace(@"&", "&#38;").Replace("\"", "&quot;") + @"</SlideHeader>");
			result.AppendLine(@"<Logo1>" + Convert.ToBase64String((byte[])converter.ConvertTo(Logo1, typeof(byte[]))).Replace(@"&", "&#38;").Replace("\"", "&quot;") + @"</Logo1>");
			result.AppendLine(@"<Logo2>" + Convert.ToBase64String((byte[])converter.ConvertTo(Logo2, typeof(byte[]))).Replace(@"&", "&#38;").Replace("\"", "&quot;") + @"</Logo2>");
			result.AppendLine(@"<Logo3>" + Convert.ToBase64String((byte[])converter.ConvertTo(Logo3, typeof(byte[]))).Replace(@"&", "&#38;").Replace("\"", "&quot;") + @"</Logo3>");
			result.AppendLine(@"<Logo4>" + Convert.ToBase64String((byte[])converter.ConvertTo(Logo4, typeof(byte[]))).Replace(@"&", "&#38;").Replace("\"", "&quot;") + @"</Logo4>");
				...
				switch (childNode.Name)
				{
					...
					case "Logo1":
						if (string.IsNullOrEmpty(childNode.InnerText))
							...
							Logo1 = new Bitmap(new MemoryStream(Convert.FromBase64String(childNode.InnerText)));
						break;
					case "Logo2":
						if (string.IsNullOrEmpty(childNode.InnerText))
							Logo2 = null;
						else
							Logo2 = new Bitmap(new MemoryStream(Convert.FromBase64String(childNode.InnerText)));
						break;
					case "Logo3":
						if (string.IsNullOrEmpty(childNode.InnerText))
							Logo3 = null;
						else
							Logo3 = new Bitmap(new MemoryStream(Convert.FromBase64String(childNode.InnerText)));
						break;
					case "Logo4":
						if (string.IsNullOrEmpty(childNode.InnerText))
							Logo4 = null;
						else
							Logo4 = new Bitmap(new MemoryStream(Convert.FromBase64String(childNode.InnerText)));
						break;
				}
			}
		}
	...
	public class MonthCalendarViewSettings
	{
		...
		public MonthViewControl MonthView { get; set; }
