...
namespace NewBizWiz.Calendar.Controls.InteropClasses
{
	...
	{
		public void AppendCalendar(CalendarOutputData monthOutputData, Presentation destinationPresentation = null)
		{
			if (Directory.Exists(BusinessWrapper.Instance.OutputManager.CalendarTemlatesFolderPath))
			{
				string presentationTemplatePath = Path.Combine(BusinessWrapper.Instance.OutputManager.CalendarTemlatesFolderPath, BusinessWrapper.Instance.OutputManager.GetSlideName(monthOutputData));
					...
					try
					{
						var thread = new Thread(delegate()
						{
							...
							foreach (Slide slide in presentation.Slides)
							{
										...
										switch (shape.Tags.Name(i))
										{
											case "LOGO":
												if (!string.IsNullOrEmpty(monthOutputData.LogoFile))
													slide.Shapes.AddPicture(FileName: monthOutputData.LogoFile, LinkToFile: MsoTriState.msoFalse, SaveWithDocument: MsoTriState.msoCTrue, Left: shape.Left, Top: shape.Top, Width: shape.Width, Height: shape.Height);
								...
								foreach (CalendarNote note in monthOutputData.Notes)
								{
									Shape noteShape = slide.Shapes.AddTextbox(MsoTextOrientation.msoTextOrientationHorizontal, note.Left, note.Top, note.Right - note.Left, note.Height);
							...
							string backgroundFilePath = Path.Combine(BusinessWrapper.Instance.OutputManager.CalendarBackgroundFolderPath, String.Format(OutputManager.BackgroundFilePath, monthOutputData.SlideColor, monthOutputData.Parent.Date.ToString("yyyy")), monthOutputData.BackgroundFileName);
		...
		private void SetDayRecordTagValue(CalendarOutputData monthOutputData, Slide slide, Shape shape, int dayNumber)
		{
			try
			{
				foreach (CalendarNote note in monthOutputData.Notes.Where(x => x.StartDay.Day == dayNumber))
				{
				...
				foreach (CalendarNote note in monthOutputData.Notes.Where(x => x.FinishDay.Day == dayNumber))
				{
					note.Right = shape.Left + shape.Width - 10;
				}

				CalendarNote midleNote = monthOutputData.Notes.Where(x => x.StartDay.Day < dayNumber && x.FinishDay.Day >= dayNumber).FirstOrDefault();
				if (midleNote != null)
				{
				...
				{
					imageShape = slide.Shapes.AddPicture(FileName: monthOutputData.DayLogoPaths[dayNumber - 1], LinkToFile: MsoTriState.msoFalse, SaveWithDocument: MsoTriState.msoCTrue, Left: shape.Left + 3, Top: shape.Top + 3);
				}
		...
		public void PrepareCalendarEmail(string fileName, CalendarOutputData[] monthOutputData)
		{
			try
			{
				...
				foreach (CalendarOutputData month in monthOutputData)
				{
