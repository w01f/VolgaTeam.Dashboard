...
{
	[ToolboxItem(false)]
	public sealed partial class InvestmentTabEControl : ChildTabBaseControl
	{
		private InvestmentTabEInfo CustomTabInfo => (InvestmentTabEInfo)TabInfo;

		public InvestmentTabEControl(IChildTabPageContainer slideContainer, ShiftChildTabInfo tabInfo) : base(slideContainer, tabInfo)
		{
			InitializeComponent();
		...
		public override void LoadData()
		{
			...
			clipartEditContainer1.LoadData(SlideContainer.EditedContent.InvestmentState.TabE.Clipart1);
			clipartEditContainer2.LoadData(SlideContainer.EditedContent.InvestmentState.TabE.Clipart2);
			clipartEditContainer3.LoadData(SlideContainer.EditedContent.InvestmentState.TabE.Clipart3);

			comboBoxEditSlideHeader.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.SlideHeader ??
				CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault);

			comboBoxEditCombo1.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo1 ??
											   CustomTabInfo.Combo1Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo2.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo2 ??
										   CustomTabInfo.Combo2Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo3.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo3 ??
										   CustomTabInfo.Combo3Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo4.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo4 ??
										   CustomTabInfo.Combo4Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo5.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo5 ??
										   CustomTabInfo.Combo5Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo6.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo6 ??
										   CustomTabInfo.Combo6Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo7.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo7 ??
										   CustomTabInfo.Combo7Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo8.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Combo8 ??
			                               CustomTabInfo.Combo8Items.FirstOrDefault(item => item.IsDefault);

			memoEditSubheader1.EditValue = SlideContainer.EditedContent.InvestmentState.TabE.Subheader1 ??
		...
		public override void ApplyChanges()
		{
			...
			SlideContainer.EditedContent.InvestmentState.TabE.Clipart1 = clipartEditContainer1.GetActiveClipartObject();
			SlideContainer.EditedContent.InvestmentState.TabE.Clipart2 = clipartEditContainer2.GetActiveClipartObject();
			SlideContainer.EditedContent.InvestmentState.TabE.Clipart3 = clipartEditContainer3.GetActiveClipartObject();
			...
			SlideContainer.EditedContent.InvestmentState.TabE.SlideHeader = slideHeaderValue != CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault) ?
				slideHeaderValue :
				null;

			SlideContainer.EditedContent.InvestmentState.TabE.Combo1 = CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo1.EditValue ?
				comboBoxEditCombo1.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo1.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo2 = CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo2.EditValue ?
				comboBoxEditCombo2.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo2.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo3 = CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo3.EditValue ?
				comboBoxEditCombo3.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo3.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo4 = CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo4.EditValue ?
				comboBoxEditCombo4.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo4.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo5 = CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo5.EditValue ?
				comboBoxEditCombo5.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo5.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo6 = CustomTabInfo.Combo6Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo6.EditValue ?
				comboBoxEditCombo6.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo6.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo7 = CustomTabInfo.Combo7Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo7.EditValue ?
				comboBoxEditCombo7.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo7.EditValue as String } :
				null;
			SlideContainer.EditedContent.InvestmentState.TabE.Combo8 = CustomTabInfo.Combo8Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo8.EditValue ?
				comboBoxEditCombo8.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo8.EditValue as String } :
				null;

			SlideContainer.EditedContent.InvestmentState.TabE.Subheader1 =
		...
		{
			return SlideContainer.EditedContent.InvestmentState.TabE.EnableOutput ?? CustomTabInfo.EnableOutput;
		}

		public override void ApplyOutputEnableState(bool outputEnabled)
		{
			SlideContainer.EditedContent.InvestmentState.TabE.EnableOutput =
		...
		protected override OutputDataPackage GetOutputData()
		{
			...
			var clipart1 = SlideContainer.EditedContent.InvestmentState.TabE.Clipart1 ??
						  ImageClipartObject.FromImage(CustomTabInfo.Clipart1Image);
			if (clipart1 != null)
			{
				clipart1.OutputBackground = true;
				outputDataPackage.ClipartItems.Add("SHIFT12ECLIPART1", clipart1);
			}

			var clipart2 = SlideContainer.EditedContent.InvestmentState.TabE.Clipart2 ??
						   ImageClipartObject.FromImage(CustomTabInfo.Clipart2Image);
			if (clipart2 != null)
				outputDataPackage.ClipartItems.Add("SHIFT12ECLIPART2", clipart2);

			var clipart3 = SlideContainer.EditedContent.InvestmentState.TabE.Clipart3 ??
						   ImageClipartObject.FromImage(CustomTabInfo.Clipart3Image);
			if (clipart3 != null)
				outputDataPackage.ClipartItems.Add("SHIFT12ECLIPART3", clipart3);

			var slideHeader = (SlideContainer.EditedContent.InvestmentState.TabE.SlideHeader ??
				CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault))?.Value;

			var combos = new[]
				{
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo1 ?? CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo2 ?? CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo3 ?? CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo4 ?? CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo5 ?? CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo6 ?? CustomTabInfo.Combo6Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo7 ?? CustomTabInfo.Combo7Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.InvestmentState.TabE.Combo8 ?? CustomTabInfo.Combo8Items.FirstOrDefault(h => h.IsDefault))?.Value,
				}
				.Where(item => !String.IsNullOrWhiteSpace(item))
				.ToList();

			outputDataPackage.TemplateName =
				MasterWizardManager.Instance.SelectedWizard.GetShiftInvestmentFile("proof_e.pptx");

			outputDataPackage.TextItems.Add("SHIFT12EHeader".ToUpper(), slideHeader);
			outputDataPackage.TextItems.Add("SHIFT12ESUBHEADER1".ToUpper(), SlideContainer.EditedContent.InvestmentState.TabE.Subheader1 ?? CustomTabInfo.SubHeader1DefaultValue);

			outputDataPackage.TextItems.Add("SHIFT12ECOMBOMERGE1".ToUpper(), String.Join(String.Format("{0}", (char)13), combos));
