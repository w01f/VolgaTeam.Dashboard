...
namespace NewBizWiz.CommonGUI.ToolForms
{
	public partial class FormEmail : Form
	{
		...
		public FormEmail(Form parentForm, IPowerPointHelper powerPointHelper, HelpManager helpManager)
		{
			InitializeComponent();
			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							_powerPointHelper.CreateLockedPresentation(PresentationFile.Replace(Path.GetExtension(PresentationFile), string.Empty), emailFile);
			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							_powerPointHelper.ConvertToPDF(PresentationFile, emailFile);
							if (File.Exists(emailFile))
							{
								if (OutlookHelper.Instance.Open())
								{
