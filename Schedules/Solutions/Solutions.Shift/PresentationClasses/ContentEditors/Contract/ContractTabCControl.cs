...
{
	[ToolboxItem(false)]
	public sealed partial class ContractTabCControl : ChildTabBaseControl
	{
		private ContractTabCInfo CustomTabInfo => (ContractTabCInfo)TabInfo;

		public ContractTabCControl(IChildTabPageContainer slideContainer, ShiftChildTabInfo tabInfo) : base(slideContainer, tabInfo)
		{
			InitializeComponent();
		...
		public override void LoadData()
		{
			...
			clipartEditContainer1.LoadData(SlideContainer.EditedContent.ContractState.TabC.Clipart1);
			clipartEditContainer2.LoadData(SlideContainer.EditedContent.ContractState.TabC.Clipart2);
			clipartEditContainer3.LoadData(SlideContainer.EditedContent.ContractState.TabC.Clipart3);

			comboBoxEditSlideHeader.EditValue = SlideContainer.EditedContent.ContractState.TabC.SlideHeader ??
				CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault);

			comboBoxEditCombo1.EditValue = SlideContainer.EditedContent.ContractState.TabC.Combo1 ??
											   CustomTabInfo.Combo1Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo2.EditValue = SlideContainer.EditedContent.ContractState.TabC.Combo2 ??
										   CustomTabInfo.Combo2Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo3.EditValue = SlideContainer.EditedContent.ContractState.TabC.Combo3 ??
										   CustomTabInfo.Combo3Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo4.EditValue = SlideContainer.EditedContent.ContractState.TabC.Combo4 ??
										   CustomTabInfo.Combo4Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo5.EditValue = SlideContainer.EditedContent.ContractState.TabC.Combo5 ??
										   CustomTabInfo.Combo5Items.FirstOrDefault(item => item.IsDefault);

			memoEditSubheader1.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader1 ??
										   CustomTabInfo.SubHeader1DefaultValue;
			memoEditSubheader2.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader2 ??
										   CustomTabInfo.SubHeader2DefaultValue;
			memoEditSubheader3.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader3 ??
										   CustomTabInfo.SubHeader3DefaultValue;
			memoEditSubheader4.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader4 ??
										   CustomTabInfo.SubHeader4DefaultValue;
			memoEditSubheader5.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader5 ??
										   CustomTabInfo.SubHeader5DefaultValue;
			memoEditSubheader6.EditValue = SlideContainer.EditedContent.ContractState.TabC.Subheader6 ??
		...
		public override void ApplyChanges()
		{
			...
			SlideContainer.EditedContent.ContractState.TabC.Clipart1 = clipartEditContainer1.GetActiveClipartObject();
			SlideContainer.EditedContent.ContractState.TabC.Clipart2 = clipartEditContainer2.GetActiveClipartObject();
			SlideContainer.EditedContent.ContractState.TabC.Clipart3 = clipartEditContainer3.GetActiveClipartObject();
			...
			SlideContainer.EditedContent.ContractState.TabC.SlideHeader = slideHeaderValue != CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault) ?
				slideHeaderValue :
				null;

			SlideContainer.EditedContent.ContractState.TabC.Combo1 = CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo1.EditValue ?
				comboBoxEditCombo1.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo1.EditValue as String } :
				null;
			SlideContainer.EditedContent.ContractState.TabC.Combo2 = CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo2.EditValue ?
				comboBoxEditCombo2.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo2.EditValue as String } :
				null;
			SlideContainer.EditedContent.ContractState.TabC.Combo3 = CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo3.EditValue ?
				comboBoxEditCombo3.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo3.EditValue as String } :
				null;
			SlideContainer.EditedContent.ContractState.TabC.Combo4 = CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo4.EditValue ?
				comboBoxEditCombo4.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo4.EditValue as String } :
				null;
			SlideContainer.EditedContent.ContractState.TabC.Combo5 = CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo5.EditValue ?
				comboBoxEditCombo5.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo5.EditValue as String } :
				null;

			SlideContainer.EditedContent.ContractState.TabC.Subheader1 =
				memoEditSubheader1.EditValue as String != CustomTabInfo.SubHeader1DefaultValue
					...
					: null;
			SlideContainer.EditedContent.ContractState.TabC.Subheader2 =
				memoEditSubheader2.EditValue as String != CustomTabInfo.SubHeader2DefaultValue
					...
					: null;
			SlideContainer.EditedContent.ContractState.TabC.Subheader3 =
				memoEditSubheader3.EditValue as String != CustomTabInfo.SubHeader3DefaultValue
					...
					: null;
			SlideContainer.EditedContent.ContractState.TabC.Subheader4 =
				memoEditSubheader4.EditValue as String != CustomTabInfo.SubHeader4DefaultValue
					...
					: null;
			SlideContainer.EditedContent.ContractState.TabC.Subheader5 =
				memoEditSubheader5.EditValue as String != CustomTabInfo.SubHeader5DefaultValue
					...
					: null;
			SlideContainer.EditedContent.ContractState.TabC.Subheader6 =
		...
		{
			return SlideContainer.EditedContent.ContractState.TabC.EnableOutput ?? CustomTabInfo.EnableOutput;
		}

		public override void ApplyOutputEnableState(bool outputEnabled)
		{
			SlideContainer.EditedContent.ContractState.TabC.EnableOutput =
		...
		protected override OutputDataPackage GetOutputData()
		{
			...
			var clipart1 = SlideContainer.EditedContent.ContractState.TabC.Clipart1 ??
						  ImageClipartObject.FromImage(CustomTabInfo.Clipart1Image);
			if (clipart1 != null)
				outputDataPackage.ClipartItems.Add("SHIFT15CCLIPART1", clipart1);

			var clipart2 = SlideContainer.EditedContent.ContractState.TabC.Clipart2 ??
						   ImageClipartObject.FromImage(CustomTabInfo.Clipart2Image);
			if (clipart2 != null)
				outputDataPackage.ClipartItems.Add("SHIFT15CCLIPART2", clipart2);

			var clipart3 = SlideContainer.EditedContent.ContractState.TabC.Clipart3 ??
			               ImageClipartObject.FromImage(CustomTabInfo.Clipart3Image);
			if (clipart3 != null)
				outputDataPackage.ClipartItems.Add("SHIFT15CCLIPART3", clipart3);

			var slideHeader = (SlideContainer.EditedContent.ContractState.TabC.SlideHeader ??
				CustomTabInfo.HeadersItems.FirstOrDefault(h => h.IsDefault))?.Value;

			var combos = new[]
				{
					(SlideContainer.EditedContent.ContractState.TabC.Combo1 ?? CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.ContractState.TabC.Combo2 ?? CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.ContractState.TabC.Combo3 ?? CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.ContractState.TabC.Combo4 ?? CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.ContractState.TabC.Combo5 ?? CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault))?.Value,
				}
				.Where(item => !String.IsNullOrWhiteSpace(item))
				.ToList();

			outputDataPackage.TemplateName =
				MasterWizardManager.Instance.SelectedWizard.GetShiftContractFile("c_agreement_next_steps.pptx");

			outputDataPackage.TextItems.Add("SHIFT15CHeader".ToUpper(), slideHeader);

			outputDataPackage.TextItems.Add("SHIFT15CCOMBOMERGE1".ToUpper(), String.Join(String.Format("{0}{0}", (char)13), combos.Select(item => String.Format("- {0}", item))));

			outputDataPackage.TextItems.Add("SHIFT15CTAB1SUBHEADER1".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader1 ?? CustomTabInfo.SubHeader1DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT15CTAB2SUBHEADER1".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader2 ?? CustomTabInfo.SubHeader2DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT15CTAB3SUBHEADER1".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader3 ?? CustomTabInfo.SubHeader3DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT15CTAB4SUBHEADER1".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader4 ?? CustomTabInfo.SubHeader4DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT15CTAB5SUBHEADER5".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader5 ?? CustomTabInfo.SubHeader5DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT15CTAB6SUBHEADER6".ToUpper(), SlideContainer.EditedContent.ContractState.TabC.Subheader6 ?? CustomTabInfo.SubHeader6DefaultValue);
