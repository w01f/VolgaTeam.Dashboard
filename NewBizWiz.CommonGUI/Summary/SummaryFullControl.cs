...
namespace NewBizWiz.CommonGUI.Summary
{
	public abstract class SummaryFullControl : SummaryBaseControl<SummaryCustomItemControl>
	{
		...
		{
			get { return checkEditMonthlyInvestment.Checked && (Schedule.CustomSummary.MonthlyValue.HasValue || Schedule.CustomSummary.TotalMonthly.HasValue) ? (Schedule.CustomSummary.MonthlyValue.HasValue ? Schedule.CustomSummary.MonthlyValue.Value : Schedule.CustomSummary.TotalMonthly).Value.ToString("$#,##0.00") : string.Empty; }
		}
		...
		{
			spinEditMonthly.EditValue = Schedule.CustomSummary.MonthlyValue.HasValue ? Schedule.CustomSummary.MonthlyValue.Value : Schedule.CustomSummary.TotalMonthly;
		...
		private void spinEditMonthly_EditValueChanged(object sender, EventArgs e)
		{
			...
			Schedule.CustomSummary.MonthlyValue = Schedule.CustomSummary.TotalMonthly != spinEditMonthly.Value ? spinEditMonthly.Value : (decimal?)null;
			SettingsNotSaved = true;
		}
		...
		{
			if (!_allowToSave) return;
			Schedule.CustomSummary.TotalValue = Schedule.CustomSummary.TotalTotal != spinEditTotal.Value ? spinEditTotal.Value : (decimal?)null;
			SettingsNotSaved = true;
		}
