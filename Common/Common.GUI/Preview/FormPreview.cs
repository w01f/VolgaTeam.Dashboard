...
namespace Asa.Common.GUI.Preview
{
    public partial class FormPreview : MetroForm
    {
        ...
        {
            previewControl.ShowPreviewControl();
        }
        ...
        private void OnOutputToggled(object sender, EventArgs e)
        {
            ...
            if (toggleSwitchOutput.IsOn)
                SelectedGroupControl.SelectedPreviewControl.EnableSlide();
            else
                SelectedGroupControl.SelectedPreviewControl.DisableSlide();
            CalculateSlides();
        }

        private void OnOutputToggleChanging(object sender, ChangingEventArgs e)
        {
            if (!_allowHandleButtonEvents) return;

            var cancel = !String.Equals(e.NewValue?.ToString(), "true", StringComparison.OrdinalIgnoreCase) &&
                         xtraTabControlGroups.TabPages
                                        .OfType<PreviewGroupControl>()
                                        .SelectMany(groupControl => groupControl.OutputItems).Count(item => item.Enabled) == 1;

            if (cancel)
                PopupMessageHelper.Instance.ShowWarning("You need to enable at least one slide");
