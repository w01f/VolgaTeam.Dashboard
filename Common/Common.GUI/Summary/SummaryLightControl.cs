...
namespace Asa.Common.GUI.Summary
{
	public abstract class SummaryLightControl : SummaryBaseControl<SummaryProductItemControl>
	{
		...
		public override string TotalMonthlyValue
		{
			get
			{
				return checkEditMonthlyInvestment.Checked && Schedule.ProductSummary.MonthlyValue.HasValue ? 
					Schedule.ProductSummary.MonthlyValue.Value.ToString("$#,##0.00") : 
					String.Empty;
			}
		}
		...
		public override string TotalTotalValue
		{
			get
			{
				return checkEditTotalInvestment.Checked && Schedule.ProductSummary.TotalValue.HasValue ? 
					Schedule.ProductSummary.TotalValue.Value.ToString("$#,##0.00") : 
					String.Empty;
			}
		}
