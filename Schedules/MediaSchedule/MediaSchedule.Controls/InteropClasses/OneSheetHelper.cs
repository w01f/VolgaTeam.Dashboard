...
namespace Asa.Media.Controls.InteropClasses
{
	...
	{
		#region Media
		public void AppendMediaOneSheet(IEnumerable<OutputMediaData> pages, Theme selectedTheme, bool pasteToSlideMaster, Presentation destinationPresentation = null)
		{
			try
			{
						...
						foreach (var shape in taggedSlide.Shapes.OfType<Shape>().Where(s => s.HasTable != MsoTriState.msoTrue))
						{
							for (var i = 1; i <= shape.Tags.Count; i++)
							{
								...
								for (var j = 0; j < page.ProgramsPerSlide; j++)
								{
									...
									{
										var fileName = page.MediaLogos.Length > j ? page.MediaLogos[j] : String.Empty;
								...
								for (var j = 0; j < 6; j++)
								{
									...
									{
										var fileName = page.DigitalLogos.Length > j ? page.DigitalLogos[j] : String.Empty;
										if (!String.IsNullOrEmpty(fileName) && File.Exists(fileName))
											logoShapes.Add(taggedSlide.Shapes.AddPicture(fileName, MsoTriState.msoFalse, MsoTriState.msoCTrue, shape.Left,
												shape.Top, shape.Width, shape.Height));
										shape.Visible = MsoTriState.msoFalse;
		...
		public void PrepareMediaOneSheetPdf(string targetFileName, IEnumerable<OutputMediaData> pages, Theme selectedTheme, bool pasteToSlideMaster)
		{
		...
		#region Digital
		public void AppendDigitalOneSheet(IEnumerable<OutputDigitalData> pages, Theme selectedTheme, bool pasteToSlideMaster, Presentation destinationPresentation = null)
		{
			try
			{
				var thread = new Thread(delegate ()
		...
		public void PrepareDigitalOneSheetEmail(string fileName, IEnumerable<OutputDigitalData> pages, Theme selectedTheme, bool pasteToSlideMaster)
		{
			PreparePresentation(fileName, presentation => AppendDigitalOneSheet(pages, selectedTheme, pasteToSlideMaster, presentation));
		}

		public void PrepareDigitalOneSheetPdf(string targetFileName, IEnumerable<OutputDigitalData> pages, Theme selectedTheme, bool pasteToSlideMaster)
		{
			var sourceFileName = Path.GetTempFileName();
			PreparePresentation(sourceFileName, presentation => AppendDigitalOneSheet(pages, selectedTheme, pasteToSlideMaster, presentation));
			BuildPdf(sourceFileName, targetFileName);
		}
		#endregion
	}
