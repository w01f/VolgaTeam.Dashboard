...
{
	[ToolboxItem(false)]
	public sealed partial class NextStepsTabHControl : ChildTabBaseControl
	{
		private NextStepsTabHInfo CustomTabInfo => (NextStepsTabHInfo)TabInfo;

		public NextStepsTabHControl(IChildTabPageContainer slideContainer, ShiftChildTabInfo tabInfo) : base(slideContainer, tabInfo)
		{
			InitializeComponent();
		...
		public override void LoadData()
		{
			...
			comboBoxEditCombo1.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Combo1 ??
											   CustomTabInfo.Combo1Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo2.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Combo2 ??
										   CustomTabInfo.Combo2Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo3.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Combo3 ??
										   CustomTabInfo.Combo3Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo4.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Combo4 ??
										   CustomTabInfo.Combo4Items.FirstOrDefault(item => item.IsDefault);
			comboBoxEditCombo5.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Combo5 ??
										   CustomTabInfo.Combo5Items.FirstOrDefault(item => item.IsDefault);

			memoEditSubheader1.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader1 ??
										   CustomTabInfo.SubHeader1DefaultValue;
			memoEditSubheader2.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader2 ??
										   CustomTabInfo.SubHeader2DefaultValue;
			memoEditSubheader3.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader3 ??
										   CustomTabInfo.SubHeader3DefaultValue;
			memoEditSubheader4.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader4 ??
										   CustomTabInfo.SubHeader4DefaultValue;
			memoEditSubheader5.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader5 ??
										   CustomTabInfo.SubHeader5DefaultValue;
			memoEditSubheader6.EditValue = SlideContainer.EditedContent.NextStepsState.TabH.Subheader6 ??
		...
		public override void ApplyChanges()
		{
			...
			SlideContainer.EditedContent.NextStepsState.TabH.Combo1 = CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo1.EditValue ?
				comboBoxEditCombo1.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo1.EditValue as String } :
				null;
			SlideContainer.EditedContent.NextStepsState.TabH.Combo2 = CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo2.EditValue ?
				comboBoxEditCombo2.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo2.EditValue as String } :
				null;
			SlideContainer.EditedContent.NextStepsState.TabH.Combo3 = CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo3.EditValue ?
				comboBoxEditCombo3.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo3.EditValue as String } :
				null;
			SlideContainer.EditedContent.NextStepsState.TabH.Combo4 = CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo4.EditValue ?
				comboBoxEditCombo4.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo4.EditValue as String } :
				null;
			SlideContainer.EditedContent.NextStepsState.TabH.Combo5 = CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault) != comboBoxEditCombo5.EditValue ?
				comboBoxEditCombo5.EditValue as ListDataItem ?? new ListDataItem { Value = comboBoxEditCombo5.EditValue as String } :
				null;

			SlideContainer.EditedContent.NextStepsState.TabH.Subheader1 =
				memoEditSubheader1.EditValue as String != CustomTabInfo.SubHeader1DefaultValue
					...
					: null;
			SlideContainer.EditedContent.NextStepsState.TabH.Subheader2 =
				memoEditSubheader2.EditValue as String != CustomTabInfo.SubHeader2DefaultValue
					...
					: null;
			SlideContainer.EditedContent.NextStepsState.TabH.Subheader3 =
				memoEditSubheader3.EditValue as String != CustomTabInfo.SubHeader3DefaultValue
					...
					: null;
			SlideContainer.EditedContent.NextStepsState.TabH.Subheader4 =
				memoEditSubheader4.EditValue as String != CustomTabInfo.SubHeader4DefaultValue
					...
					: null;
			SlideContainer.EditedContent.NextStepsState.TabH.Subheader5 =
				memoEditSubheader5.EditValue as String != CustomTabInfo.SubHeader5DefaultValue
					...
					: null;
			SlideContainer.EditedContent.NextStepsState.TabH.Subheader6 =
		...
		{
			return SlideContainer.EditedContent.NextStepsState.TabH.EnableOutput ?? CustomTabInfo.EnableOutput;
		}

		public override void ApplyOutputEnableState(bool outputEnabled)
		{
			SlideContainer.EditedContent.NextStepsState.TabH.EnableOutput =
		...
		protected override OutputDataPackage GetOutputData()
		{
			...
			var clipart1 = SlideContainer.EditedContent.NextStepsState.TabH.Clipart1 ??
						  ImageClipartObject.FromImage(CustomTabInfo.Clipart1Image);
			if (clipart1 != null)
				outputDataPackage.ClipartItems.Add("SHIFT14HCLIPART1", clipart1);

			var clipart2 = SlideContainer.EditedContent.NextStepsState.TabH.Clipart2 ??
			...
			var combos = new[]
				{
					(SlideContainer.EditedContent.NextStepsState.TabH.Combo1 ?? CustomTabInfo.Combo1Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.NextStepsState.TabH.Combo2 ?? CustomTabInfo.Combo2Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.NextStepsState.TabH.Combo3 ?? CustomTabInfo.Combo3Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.NextStepsState.TabH.Combo4 ?? CustomTabInfo.Combo4Items.FirstOrDefault(h => h.IsDefault))?.Value,
					(SlideContainer.EditedContent.NextStepsState.TabH.Combo5 ?? CustomTabInfo.Combo5Items.FirstOrDefault(h => h.IsDefault))?.Value,
				}
				.Where(item => !String.IsNullOrWhiteSpace(item))
				.ToList();

			outputDataPackage.TemplateName =
				MasterWizardManager.Instance.SelectedWizard.GetShiftNextStepsFile("h_next_steps.pptx");

			outputDataPackage.TextItems.Add("SHIFT14HHeader".ToUpper(), slideHeader);

			outputDataPackage.TextItems.Add("SHIFT14HCOMBOMERGE1".ToUpper(), String.Join(String.Format("{0}{0}", (char)13), combos.Select(item => String.Format("- {0}", item))));

			outputDataPackage.TextItems.Add("SHIFT14HTAB1SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader1 ?? CustomTabInfo.SubHeader1DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT14HTAB2SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader2 ?? CustomTabInfo.SubHeader2DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT14HTAB3SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader3 ?? CustomTabInfo.SubHeader3DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT14HTAB4SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader4 ?? CustomTabInfo.SubHeader4DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT14HTAB5SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader5 ?? CustomTabInfo.SubHeader5DefaultValue);
			outputDataPackage.TextItems.Add("SHIFT14HTAB6SUBHEADER1".ToUpper(), SlideContainer.EditedContent.NextStepsState.TabH.Subheader6 ?? CustomTabInfo.SubHeader6DefaultValue);
