					...
					for (int k = 0; k < (totalSpotsCount == 0 ? 1 : totalSpotsCount); k += 13)
					{
						...
						for (int j = 0; j < programsPerSlide; j++)
						{
							if ((i + j) < _localSchedule.MonthlySchedule.Programs.Count)
							{
								...
								if (FormMain.Instance.buttonItemMonthlyScheduleRate.Checked && program.Rate.HasValue)
						...
						outputPage.TotalGRP = FormMain.Instance.buttonItemMonthlyScheduleGRP.Checked ? _localSchedule.MonthlySchedule.TotalGRP.ToString(_localSchedule.RatingAsCPP ? "#,###.0" : "#,##0") : string.Empty;
						#endregion
