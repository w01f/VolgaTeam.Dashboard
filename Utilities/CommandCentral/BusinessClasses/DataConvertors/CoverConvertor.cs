...
{
	class CoverConvertor : IExcel2XmlConvertor
	{
		private const string DestinationFileName = "Add Cover.xml";
		private readonly string _sourceFilePath;

		public CoverConvertor(string sourceFilePath)
		{
					...
					try
					{
						...
						if (columnsCount > 0)
							foreach (DataRow row in dataTable.Rows)
							{
								...
								if (!String.IsNullOrEmpty(value) && !String.IsNullOrEmpty(author) && !value.StartsWith("*",StringComparison.OrdinalIgnoreCase))
