...
namespace AdScheduleBuilder.OutputClasses.OutputControls
{
	...
	public partial class OutputSnapshotControl : UserControl, ISummaryOutputControl
	{
		...
		{
			HelpManager.Instance.OpenHelpLink("snapshot");
		}
		#endregion
		...
		{
			_allowToSave = false;
			FormMain.Instance.buttonItemSnapshotOptions.Checked = LocalSchedule.ViewSettings.SnapshotViewSettings.ShowOptions;
			...
			buttonXPercentOfPage.Enabled = LocalSchedule.ViewSettings.SnapshotViewSettings.EnablePercentOfPage & ListManager.Instance.ShareUnits.Count > 0;
		...
		private void SaveView()
		{
			if (_allowToSave)
			{
				LocalSchedule.ViewSettings.SnapshotViewSettings.ShowOptions = FormMain.Instance.buttonItemSnapshotOptions.Checked;
		...
		{
			HelpManager.Instance.OpenHelpLink("snapshotnavbar");
		}
		#endregion
		...
		{
			get { return buttonXLogo.Checked ? 1 : 2; }
		}
		...
		public string[] LogoFiles
		{
			get
			{
				...
				foreach (Publication publication in LocalSchedule.Publications.Where(x => x.Inserts.Count > 0))
				{
		...
		public void PrintOutput()
		{
			using (var formProgress = new FormProgress())
			{
				...
				formProgress.Show();
				PowerPointHelper.Instance.AppendSnapshot();
				formProgress.Close();
			}
		...
		public void Email()
		{
			using (var formProgress = new FormProgress())
			{
				...
				PowerPointHelper.Instance.PrepareSnapshotEmail(tempFileName);
				formProgress.Close();
