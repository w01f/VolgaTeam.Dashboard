			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							AdSchedulePowerPointHelper.Instance.CreateLockedPresentation(PresentationFile.Replace(Path.GetExtension(PresentationFile), string.Empty), emailFile);
			...
			if (!string.IsNullOrEmpty(PresentationFile))
			{
				using (var form = new FormEmailFileName())
				{
					...
					if (form.ShowDialog() == DialogResult.OK)
					{
						...
						try
						{
							AdSchedulePowerPointHelper.Instance.ConvertToPDF(PresentationFile, emailFile);
							if (File.Exists(emailFile))
							{
								if (OutlookHelper.Instance.Open())
								{
