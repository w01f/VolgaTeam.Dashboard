...
namespace AdScheduleBuilder.InteropClasses
{
	public partial class PowerPointHelper
	{
		public void AppendCalendar(MonthViewControl monthView, Presentation destinationPresentation = null)
		{
			...
			{
				string presentationTemplatePath = Path.Combine(OutputManager.Instance.CalendarTemlatesFolderPath, monthView.SlideName);
						...
						var thread = new Thread(delegate()
													{
														...
														if (excelHelper.Connect())
														{
															...
															{
																ShapeRange shapeRange = presentation.SlideMaster.Shapes.PasteSpecial(PpPasteDataType.ppPasteEnhancedMetafile, MsoTriState.msoFalse, "", 0, "", MsoTriState.msoFalse);
																// Resize selection
	...
	{
		public bool GetCalendarBackground(MonthViewControl monthView)
		{
			bool result = false;
			try
			{
				Workbook workBook = _excelObject.Workbooks.Open(Path.Combine(OutputManager.Instance.CalendarBackgroundFolderPath, string.Format(OutputManager.ExcelBackgroundFileName, new object[] { monthView.SlideColor, monthView.Month.ToString("yyyy") })));
