	...
	public abstract partial class GalleryControl : UserControl
	{
		...
		void WebView_Command(object sender, CommandEventArgs e)
		{
			...
			else if (e.CommandId == _idCommandDownload)
			{
				var images = DownloadImages(new[] { e.MenuInfo.SourceUrl });
				if (images.Any())
					Utilities.Instance.PutImageToClipboard(images.First().Value);
			}
		}
		...
		private void AddToFavorites(Dictionary<string, Image> images)
		{
			...
			Utilities.Instance.ShowInformation("Images successfully added to Favorites");
		}
		...
		{
			Utilities.Instance.PutImageToClipboard(_imageContainer.Image.GetAsBitmap());
		}
